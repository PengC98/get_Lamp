...
#include <algorithm>
#include "time.h"
const String fileName = "../res/my_photo-51.jpg";
...
void AutoAim::setImage(Mat &img, Mat &mask){
    ...
    //morphologyEx(channel[0]-channel[2], mask, MORPH_BLACKHAT, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(19, 19), cv::Point(-1, -1)));
    imshow("mask1", channel[0]-channel[1]);
    //开运算消除小物块，平滑物体的边界
...
void AutoAim::findLamp(Mat &mask, vector<RotatedRect> &lamps){
    ...
    for(i=0; i<size; i++){
        ...
        for(j=1; j<=3 && (i+j)<size; j++){
            ...
            if(ratio < 1.0 || ratio > 12) continue;
...
int main(int argc, char const *argv[]){
    ...
    for(int i=0;i<lamps.size();i++)
    {
        ...
        circle(src,lamps[i].center,20,(0,255,0),2);
    }
    imshow("src",src);
    //finish=clock();
